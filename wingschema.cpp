...
printNode(int nodeId, bool recurs)
{
    ...
    //dataFile.write( << format("{{ \"{}\", {:10d} }},\n", fullname, def.id);
    dataFile.put((def.id >> 24) & 0xff);
    dataFile.put((def.id >> 16) & 0xff);
    dataFile.put((def.id >>  8) & 0xff);
    dataFile.put((def.id      ) & 0xff);
    dataFile.put(fullname.size());
    for (int i = 0; i < fullname.size(); i++) {
        dataFile.put(fullname[i]);
    }
...
main()
{
    ...
    dataFile.open("wing-schema.map", ios_base::trunc);
